...
function adapterFrom(ctx: { readonly context: { readonly adapter?: unknown } }): RegistrationAdapter {
  return ctx.context.adapter as RegistrationAdapter;
}

function sessionUser(session: { readonly user?: unknown } | null | undefined): SessionUser {
  const user = session?.user as { readonly id?: unknown; readonly role?: unknown } | undefined;
  if (typeof user?.id !== "string") throw new APIError("UNAUTHORIZED");
  return { id: user.id, role: user.role };
}

function requestedOrganizationId(query: Record<string, unknown> | undefined): string | null | undefined {
  if (!query || !("organizationId" in query)) return undefined;
  return typeof query.organizationId === "string" && query.organizationId ? query.organizationId : null;
}
...
async function requirePolicyAccess(
  ...
  await assertRegistrationAdminAccess(options.authorize, organizationId, user.id, user.role, adapter);
  return user;
}
...
async function requirePolicyMoveAccess(
  ...
  const user = await requirePolicyAccess(options, adapter, session, existingOrganizationId);
  if (targetOrganizationId !== existingOrganizationId) {
    await assertRegistrationAdminAccess(options.authorize, targetOrganizationId, user.id, user.role, adapter);
  }
  return user;
}

async function findPolicy(adapter: RegistrationAdapter, id: string | undefined): Promise<RegistrationPolicyRow> {
...
const evaluateMetadata = registrationEndpointMeta({
  description: "Evaluate a public registration request and create an intent when allowed",
...
const submitMetadata = registrationEndpointMeta({
  description: "Preflight a registration form submit and reserve soft quota before Better Auth signup",
...
export const idRegistration = (options: RegistrationPluginOptions = {}): BetterAuthPlugin => ({
  id: "id-registration",
  schema: {
    registrationPolicy: {
  ...
  endpoints: {
    createRegistrationPolicy: createAuthEndpoint(
      ...
      { method: "POST", use: [sessionMiddleware], body: createRegistrationPolicyBody, metadata: createPolicyMetadata },
      async (ctx) => {
        const adapter = adapterFrom(ctx);
        const user = await requirePolicyAccess(options, adapter, ctx.context.session, ctx.body.organizationId);
    ...
    listRegistrationPolicies: createAuthEndpoint(
      ...
      async (ctx) => {
        ...
        const visible = (await Promise.all(rows.map(async (row) => {
          if (requestedOrgId !== undefined && row.organizationId !== requestedOrgId) return null;
          const allowed = await options.authorize?.(row.organizationId, user.id, user.role, adapter);
    ...
    getRegistrationPolicy: createAuthEndpoint(
      ...
      async (ctx) => {
        ...
        await requirePolicyAccess(options, adapter, ctx.context.session, policy.organizationId);
        return ctx.json(await presentPolicyWithQuota(adapter, policy));
      },
    ),
    updateRegistrationPolicy: createAuthEndpoint(
      "/admin/registration-policies/:id",
      { method: "PATCH", use: [sessionMiddleware], body: updateRegistrationPolicyBody, metadata: updatePolicyMetadata },
      async (ctx) => {
        const adapter = adapterFrom(ctx);
        const existing = await findPolicy(adapter, ctx.params?.id);
        const targetOrgId = ctx.body.organizationId === undefined ? existing.organizationId : ctx.body.organizationId;
        const user = await requirePolicyMoveAccess(options, adapter, ctx.context.session, existing.organizationId, targetOrgId);
        if (ctx.body.slug) await assertUniqueRegistrationPolicySlug(adapter, ctx.body.slug, existing.id);
    ...
    evaluateRegistration: createAuthEndpoint(
      ...
      { method: "POST", body: evaluateRegistrationBody, metadata: evaluateMetadata },
      async (ctx) => ctx.json(await evaluateRegistration(adapterFrom(ctx), ctx.body, intentTtlMs(options.intentTtlMs), ctx.context.secret)),
    ),
    submitRegistration: createAuthEndpoint(
      "/registration/submit",
      { method: "POST", body: submitRegistrationBody, metadata: submitMetadata },
      async (ctx) => ctx.json(await submitRegistration(adapterFrom(ctx), ctx.body)),
    ),
    getRegistrationStatus: createAuthEndpoint(
      "/registration/status",
      { method: "GET" },
      async (ctx) => {
        const intentId = typeof ctx.query?.intentId === "string" ? ctx.query.intentId : "";
