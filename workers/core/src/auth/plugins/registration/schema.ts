import { z } from "zod";
import { REGISTRATION_CONTINUATION_FAILURE_REASON_MAX_LENGTH } from "../../config";
import {
...
export const registrationPolicyStatusSchema = z.enum([
  "draft",
  "enabled",
  "paused",
  "archived",
...
export const registrationQuotaTargetSchema = z.enum([
  "accounts",
  "memberships",
  "verified_accounts",
]);
...
export const registrationReservationStatusSchema = z.enum([
  "reserved",
  "consumed",
  "released",
]);

export const registrationPolicySchema = z
  .object({
    id: z.string(),
    slug: z
      .string()
  ...
  .object({
    ...
    policyId: z
      ...
      .meta({
        betterAuth: {
          index: true,
          references: { model: "registrationPolicy", field: "id" },
        },
      }),
    clientId: z
      .string()
...
export const registrationQuotaReservationSchema = z
  .object({
    id: z.string(),
    policyId: z
      .string()
    ...
    intentId: z
      ...
      .meta({
        betterAuth: {
          unique: true,
          references: { model: "registrationIntent", field: "id" },
        },
      }),
    status: z
      .string()
...
export type RegistrationPolicyRow = Readonly<
  z.infer<typeof registrationPolicySchema>
>;
export type RegistrationIntentRow = Readonly<
  z.infer<typeof registrationIntentSchema>
>;
export type RegistrationQuotaReservationRow = Readonly<
  z.infer<typeof registrationQuotaReservationSchema>
>;
export type RegistrationPolicyStatus = z.infer<
  typeof registrationPolicyStatusSchema
>;

export const createRegistrationPolicyBody = z
  .object({
...
export const updateRegistrationPolicyBody = createRegistrationPolicyBody
  .partial()
  .extend({
...
export const evaluateRegistrationBody = z
  .object({
...
export const statusRegistrationBody = z
  .object({
    intentId: z.string().min(1),
  })
  .strict();

export const continuationFailureRegistrationBody = z
  .object({
    intentId: z.string().min(1),
    reason: z
      .string()
      .min(1)
      .max(REGISTRATION_CONTINUATION_FAILURE_REASON_MAX_LENGTH)
      .optional(),
  })
  .strict();

export type CreateRegistrationPolicyBody = z.infer<
  typeof createRegistrationPolicyBody
>;
export type UpdateRegistrationPolicyBody = z.infer<
  typeof updateRegistrationPolicyBody
>;
...
export type ContinuationFailureRegistrationBody = z.infer<
  typeof continuationFailureRegistrationBody
>;

export const registrationPolicyBetterAuthFields = mapZodToBetterAuthFields(
  registrationPolicySchema,
);
export const registrationIntentBetterAuthFields = mapZodToBetterAuthFields(
  registrationIntentSchema,
);
export const registrationQuotaReservationBetterAuthFields =
  mapZodToBetterAuthFields(registrationQuotaReservationSchema);

export const registrationPolicyOpenApiSchema = zodSchemaToOpenApi(
  registrationPolicySchema,
);
...
export const evaluateRegistrationOpenApiSchema = zodSchemaToOpenApi(
  z.discriminatedUnion("decision", [
    z.object({
      ...
      intentId: z.string(),
      client: z
        .object({ clientId: z.string(), clientName: z.string() })
        .nullable(),
...
export const createRegistrationPolicyOpenApiRequestBody =
  openApiJsonRequestBody(createRegistrationPolicyBody);
export const updateRegistrationPolicyOpenApiRequestBody =
  openApiJsonRequestBody(updateRegistrationPolicyBody);
export const evaluateRegistrationOpenApiRequestBody = openApiJsonRequestBody(
  evaluateRegistrationBody,
);
export const submitRegistrationOpenApiRequestBody = openApiJsonRequestBody(
