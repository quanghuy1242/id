...
export const adminActivityLogSchema = z.object({
  id: z.string().meta({ description: "Unique identifier of the activity entry" }),
  actorId: z.string().min(1).meta({
    description: "User ID or system actor ID that performed the action",
    betterAuth: { index: true },
  }),
  actorType: actorTypeSchema,
  action: z.string().min(1).meta({
    description: "Stable action name, for example oauth_client.update or user.ban",
    betterAuth: { index: true },
  }),
  targetType: z.string().min(1).meta({
    description: "Stable target type, for example oauth_client, user, jwks, or team",
    betterAuth: { index: true },
  }),
...
export type AdminActivityLogRow = Readonly<z.infer<typeof adminActivityLogSchema>>;
...
const presentedActivitySchema = z.object({
  id: z.string(),
  actorId: z.string(),
  actorType: z.string(),
...
export const adminActivityLogBetterAuthFields = mapZodToBetterAuthFields(adminActivityLogSchema);
export const listActivityLogOpenApiSchema = zodSchemaToOpenApi(
  z.object({ entries: z.array(presentedActivitySchema), total: z.number(), limit: z.number(), offset: z.number() }),
);
...
const queryParameters: QueryParameter[] = [
  { name: "targetType", in: "query", required: false, schema: { type: "string" }, description: "Filter by target type" },
  { name: "targetId", in: "query", required: false, schema: { type: "string" }, description: "Filter by target ID" },
  { name: "action", in: "query", required: false, schema: { type: "string" }, description: "Filter by action name" },
  { name: "actorId", in: "query", required: false, schema: { type: "string" }, description: "Filter by actor ID" },
