...
import {
  resolveConsoleScopeEnvelope,
  type ConsoleScopesAdapter,
} from "./operations";
...
export const idConsoleScopes = (
  options: ConsoleScopesPluginOptions = {},
): BetterAuthPlugin => ({
  ...
  endpoints: {
    getConsoleScopes: createAuthEndpoint(
      ...
      async (ctx) => {
        ...
        const platformStepUpAt = (
          session.session as { platformStepUpAt?: number | null }
        ).platformStepUpAt;
        return ctx.json(
          await resolveConsoleScopeEnvelope({
            ...
            platformStepUpSatisfied: isPlatformStepUpFresh(
