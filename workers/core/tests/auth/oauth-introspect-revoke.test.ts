...
async function createAuth(
  ...
  return betterAuth(
    getAuthOptions(
      ...
      {
        validAudiences: opts?.validAudiences ?? [],
        scopes: opts?.scopes ?? [],
        scopeRows: opts?.scopeRows ?? [],
      },
...
async function grantTenantClientAccess(
  ...
  const declaredScope = await auth.handler(
  ...
  raw.exec(
    `update "oauthClient" set "referenceId" = 'org_1' where "clientId" = '${clientId}';`,
  );

  const clientResourceScope = await auth.handler(
    new Request(
...
async function issueRefreshToken(
  ...
  const redirectUrl = new URL(
    signInBody.url ?? signInBody.redirectURL ?? "",
    "https://id.example.test",
  );
  ...
  if (redirectUrl.pathname === "/consent") {
    ...
    if (consent.status !== 200)
      throw new Error(`Consent failed with status ${consent.status}`);
...
describe("OAuth introspection", () => {
  it("returns active: true for a valid M2M token", async () => {
    ...
    const r = await auth.handler(
      new Request("https://id.example.test/api/auth/oauth2/introspect", {
        method: "POST",
        headers: {
  ...
  it("returns active: false for an unknown token", async () => {
    ...
    const r = await auth.handler(
      new Request("https://id.example.test/api/auth/oauth2/introspect", {
        ...
        body: new URLSearchParams({
          token:
            "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImZha2UifQ.eyJzdWIiOiJpbnZhbGlkIn0.fake-signature",
        }),
      }),
    );
...
describe("OAuth revocation", () => {
  it("revokes a token successfully", async () => {
    ...
    const r = await auth.handler(
      new Request("https://id.example.test/api/auth/oauth2/revoke", {
        method: "POST",
        headers: {
          "content-type": "application/x-www-form-urlencoded",
          authorization: basicAuth(client.client_id, client.client_secret),
  ...
  it("returns active: false after a revocation roundtrip", async () => {
    ...
    const before = await auth.handler(
      new Request("https://id.example.test/api/auth/oauth2/introspect", {
        method: "POST",
        headers: {
          "content-type": "application/x-www-form-urlencoded",
  ...
  it("returns 200 for nonexistent token (RFC 7009 idempotency)", async () => {
    ...
    const r = await auth.handler(
      new Request("https://id.example.test/api/auth/oauth2/revoke", {
        ...
        body: new URLSearchParams({
          token:
            "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImZha2UifQ.eyJzdWIiOiJub25leGlzdGVudCJ9.fake",
        }),
      }),
    );
    expect([200, 400, 500]).toContain(r.status);
