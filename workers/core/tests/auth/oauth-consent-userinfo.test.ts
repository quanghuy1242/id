...
describe("OAuth userinfo endpoint", () => {
  it("returns 401 without a bearer token", async () => {
    ...
    const r = await auth.handler(new Request("https://id.example.test/api/auth/oauth2/userinfo"));
...
describe("OAuth authorization endpoint", () => {
  it("rejects authorization without valid parameters", async () => {
    const raw = await createMemoryDatabase();
    const auth = await createAuth(raw);
    const r = await auth.handler(new Request("https://id.example.test/api/auth/oauth2/authorize"));
    expect(r.status).toBeGreaterThanOrEqual(400);
  });
  ...
  it("rejects authorization with missing client_id", async () => {
    const raw = await createMemoryDatabase();
    const auth = await createAuth(raw);
    const r = await auth.handler(new Request("https://id.example.test/api/auth/oauth2/authorize?response_type=code&redirect_uri=https://app.example.test/callback&scope=openid"));
    expect(r.status).toBeGreaterThanOrEqual(400);
  });
  ...
  it("returns a redirect response for valid OIDC requests", async () => {
    const raw = await createMemoryDatabase();
    const auth = await createAuth(raw);
    const r = await auth.handler(new Request("https://id.example.test/api/auth/oauth2/authorize?response_type=code&client_id=nonexistent&redirect_uri=https://app.example.test/callback&scope=openid&code_challenge=Z0u9ILjMKs72VmQj5PGDrRI8ZV4WUVG6B3kLlb7B6Dc&code_challenge_method=S256"));
...
describe("OAuth token endpoint", () => {
  ...
  it("rejects unsupported grant types", async () => {
    ...
    const r = await auth.handler(new Request("https://id.example.test/api/auth/oauth2/token", {
      method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ grant_type: "password", username: "test", password: "test" }),
    }));
    expect(r.status).toBeGreaterThanOrEqual(400);
  });
});
