...
describe("OAuth client picker endpoint", () => {
  ...
  it("reports enabled resource eligibility for a tenant client attachment", async () => {
    ...
    const lookup = await lookupTenantClient(test, token, tenant.clientId, CONTENT_AUDIENCE);
    expect(lookup.status).toBe(200);
    const body = (await lookup.json()) as Record<string, unknown>;
    expect(body.resource_access).toEqual({ resource: CONTENT_AUDIENCE, status: "enabled" });
  });

  it("reports disabled resource eligibility for disabled attachments and resource servers", async () => {
    const { test, cookie, infra } = await seedInfraInfrastructure();
    const { attachment, resourceServerId, tenant } = await seedTenantResource(test, cookie);
    const token = await issueInfraToken(test, infra);
    ...
    const disabledAttachmentLookup = await lookupTenantClient(test, token, tenant.clientId, CONTENT_AUDIENCE);
    expect(disabledAttachmentLookup.status).toBe(200);
    expect(await disabledAttachmentLookup.json()).toEqual(
      expect.objectContaining({ resource_access: { resource: CONTENT_AUDIENCE, status: "disabled" } }),
    );
    ...
    const disabledResourceLookup = await lookupTenantClient(test, token, tenant.clientId, CONTENT_AUDIENCE);
  ...
  it("returns missing resource eligibility for absent or wrong-layer attachments", async () => {
    ...
    const missingLookup = await lookupTenantClient(test, token, tenant.clientId, CONTENT_AUDIENCE);
    expect(missingLookup.status).toBe(200);
    expect(await missingLookup.json()).toEqual(
      expect.objectContaining({ resource_access: { resource: CONTENT_AUDIENCE, status: "missing" } }),
    );

    const systemLookup = await lookupTenantClient(test, token, tenant.clientId, SYSTEM_AUDIENCE);
    expect(systemLookup.status).toBe(200);
    expect(await systemLookup.json()).toEqual(
      expect.objectContaining({ resource_access: { resource: SYSTEM_AUDIENCE, status: "missing" } }),
    );
  });
