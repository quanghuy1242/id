...
import {
  authPathNeedsResourceAudiences,
  getAuthOptions,
} from "../../src/auth/get-auth";
...
const jwtPluginSource = readFileSync(
...
describe("Better Auth installed contract", () => {
  it("records the public auth route map under the configured base path", () => {
    ...
    expect(authRouteMap).not.toContainEqual(
      expect.objectContaining({ path: "/api/auth/userinfo" }),
    );
  });

  it("proves OAuth Provider endpoint names from installed package types", () => {
    expect(oauthProviderEndpointSource).toContain(
  ...
  it("proves OAuth Provider audience and custom field hook types", () => {
    ...
    expect(oauthProviderTypeSource).toContain(
  ...
  it("proves JWT plugin jwksPath default and repo override path", () => {
    expect(jwtPluginSource).toContain(
      'const jwksPath = options?.jwks?.jwksPath ?? "/jwks"',
    );
    expect(authPluginConfig.jwksPath).toBe("/jwks");
    expect(authRouteMap).toContainEqual(
      expect.objectContaining({ name: "getJwks", path: "/api/auth/jwks" }),
    );
  });
  ...
  it("constructs Better Auth options with plugin-owned resource server schema", () => {
    ...
    expect(
      options.plugins?.some((plugin) => plugin.id === "id-resource-server"),
    ).toBe(true);
    expect(
      options.plugins?.some((plugin) => plugin.id === "id-oauth-scope-catalog"),
    ).toBe(true);
    expect(
    ...
    expect(
      options.plugins?.some((plugin) => plugin.id === "oauth-provider"),
    ).toBe(true);
  });

  it("loads resource audiences only for OAuth endpoints that validate resource parameters", () => {
    expect(authPathNeedsResourceAudiences("/api/auth/oauth2/authorize")).toBe(
      true,
    );
    expect(authPathNeedsResourceAudiences("/api/auth/oauth2/token")).toBe(true);
    expect(
      authPathNeedsResourceAudiences("/api/auth/oauth2/create-client"),
    ).toBe(true);
