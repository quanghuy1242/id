...
export const usersListKey = (params: ListUsersParams) =>
  [USERS_LIST, params] as const;
...
export const orgDetailKey = (organizationId: string) =>
  [ORG_DETAIL, { organizationId }] as const;
export const orgMembersKey = (organizationId: string) =>
  [ORG_MEMBERS, { organizationId }] as const;
export const orgTeamsKey = (organizationId: string) =>
  [ORG_TEAMS, { organizationId }] as const;
export const orgInvitationsKey = (organizationId: string) =>
  [ORG_INVITATIONS, { organizationId }] as const;
export const registrationPoliciesKey = (scope: ActiveScope = platformScope) =>
  [REGISTRATION_POLICIES, scope] as const;
export const registrationPolicyIntentsKey = (policyId: string) =>
  [REGISTRATION_POLICY_INTENTS, { policyId }] as const;
...
export const oauthClientsKey = (scope: ActiveScope = platformScope) =>
  [OAUTH_CLIENTS, scope] as const;
export const resourceServersKey = (scope: ActiveScope = platformScope) =>
...
export const adminSessionsKey = (params: { limit: number; offset: number }) =>
  [ADMIN_SESSIONS, params] as const;
export const adminTokensKey = (params: {
  limit: number;
  offset: number;
  type: string;
}) => [ADMIN_TOKENS, params] as const;
export const adminConsentsKey = (params: {
  limit: number;
  offset: number;
  clientId?: string;
...
export const activityLogKey = (params: {
  ...
  limit: number;
  offset: number;
}) => [ADMIN_ACTIVITY_LOG, params] as const;

/** Matches every users-list cache slot regardless of filter/sort/page params. */
export const isUsersListKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === USERS_LIST;
/** Matches the console scope envelope cache slot. */
export const isConsoleScopesKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === CONSOLE_SCOPES;
/** Matches the organizations-list cache slot. */
export const isOrgsListKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === ORGS_LIST;
/** Matches registration policy list slots. */
export const isRegistrationPoliciesKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === REGISTRATION_POLICIES;
/** Matches the OAuth clients list cache slot. */
export const isOauthClientsKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === OAUTH_CLIENTS;
/** Matches the resource-servers list cache slot. */
export const isResourceServersKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === RESOURCE_SERVERS;
/** Matches the OAuth scopes list cache slot. */
export const isOauthScopesKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === OAUTH_SCOPES;
/** Matches the M2M bindings list cache slot. */
export const isM2mBindingsKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === OAUTH_CLIENT_RESOURCE_SCOPES;
/** Matches every activity-log cache slot regardless of target/filter/page params. */
export const isActivityLogKey = (key: Arguments) =>
  Array.isArray(key) && key[0] === ADMIN_ACTIVITY_LOG;
