...
Components:
  ...
  MobileDock: AdminMobileNav ("use client", usePathname for dock-active, lg:hidden) with one section-level entry per visible section, `dock-label` text under a compact glyph; the organization-scope overview entry is labeled Overview instead of Dash. Platform scope links only implemented platform sections; organization scope links implemented org sections, including org Security only for bounded Consents and excluding platform-only sessions, tokens, JWKS, introspection, and system routes.

Dashboard contracts:
  Platform dashboard (`/admin/platform`) uses platform-safe global reads only: `listUsers(limit=1)`, `listOrganizations`, `listClients(platform)`, `listAdminSessions(limit=1)`, `listAdminTokens(type=access|refresh, limit=1)`, `listAdminConsents(limit=1)`, and `listAdminJwks`. All shortcut links stay under `/admin/platform/**`.
  Organization overview (`/admin/orgs/:orgId`) uses organization-bounded reads only: organization detail, members, teams, invitations, clients in the active organization bridge filtered to `reference_id == orgId`, resource servers/scopes/bindings with `organizationId`, consent total with `organizationId`, and activity total with `organizationId`. Shortcut links stay under `/admin/orgs/:orgId/**`.
