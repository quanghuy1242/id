...
| `listRegistrationPolicies` | GET `/admin/registration-policies` | optional query `{ organizationId }` from the active scope | `{ policies }` | Unwraps to `RegistrationPolicy[]`; platform sends no organization param, org scope sends route-bound `organizationId`. |
| `createRegistrationPolicy` | POST `/admin/registration-policies` | flat `{ slug, name, mode, clientId?, organizationId?, resourceServerId?, allowedScopes, emailDomains, defaultRole: "member", defaultTeamIds, quotaLimit?, quotaTarget, requiresEmailVerification, startsAt?, expiresAt? }` | `RegistrationPolicy` | Creates a draft policy through the Better Auth plugin endpoint. |
| `updateRegistrationPolicy` | PATCH `/admin/registration-policies/:id` | partial create body plus optional `status` | `RegistrationPolicy` | Returns the plugin entity unchanged; UI uses this for definition edits, while enable/pause/archive use explicit transition endpoints. |
| `enableRegistrationPolicy` | POST `/admin/registration-policies/:id/enable` | `{}` | `RegistrationPolicy` | Enables admission for matching registration requests. |
| `pauseRegistrationPolicy` | POST `/admin/registration-policies/:id/pause` | `{}` | `RegistrationPolicy` | Invalidates active started/submitted intents and releases reservations before account creation can continue. |
| `archiveRegistrationPolicy` | POST `/admin/registration-policies/:id/archive` | `{}` | `RegistrationPolicy` | Operational close-switch for a retired policy; active intents are invalidated. |
| `listRegistrationPolicyIntents` | GET `/admin/registration-policies/:id/intents` | none | `{ intents }` | Unwraps to `RegistrationIntent[]` for the selected policy detail panel. |
