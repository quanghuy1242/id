...
| `listAdminConsents` | GET `/admin/list-consents` | query `{ limit, offset, clientId? }` | `{ consents, total, limit, offset }` | Returns the plugin envelope unchanged. |
| `revokeConsent` | POST `/admin/revoke-consent` | JSON `{ clientId, userId }` | `{ success: true }` | Resolves `void`. |
...
| `listActivityLog` | GET `/admin/activity-log` | query `{ limit, offset, targetType, targetId, action?, actorId? }` | `{ entries, total, limit, offset }` | Returns the plugin envelope unchanged. |
