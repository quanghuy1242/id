...
Legacy compatibility routes only. Registration policies are an Access admission-policy surface now; canonical specs and route files live in [access.md](access.md#adminplatformaccessregistration-policies-and-adminorgsorgidaccessregistration-policies). These Identity URLs redirect to `/admin/platform/access/registration-policies` and `/admin/orgs/:orgId/access/registration-policies` while preserving query params.

Components:
  Redirect only; no Identity content component.

Data: none.

Behavior:
  - Proxy and route-level redirects preserve bookmarks and query state.
  - New specs, stories, and links must use the Access routes.
