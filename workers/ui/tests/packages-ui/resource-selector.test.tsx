...
import { ResourceSelector, type ResourceOption, type ResourceSource } from "@id/ui";
...
describe("ResourceSelector", () => {
  ...
  it("calls onChange with the id on single selection", async () => {
    ...
    render(<ResourceSelector kind="member" value="" onChange={onChange} source={syncSource} />);
  ...
  it("adds to the array in multiple mode", async () => {
    const onChange = vi.fn<(next: string | string[]) => void>();
    render(
      <ResourceSelector kind="member" selectionMode="multiple" value={[]} onChange={onChange} source={syncSource} />,
    );
  ...
  it("hides excluded ids from the option list", async () => {
    render(
      <ResourceSelector kind="member" value="" onChange={() => {}} source={syncSource} excludeIds={["u1"]} />,
    );
  ...
  it("uses source labels for initially selected ids", async () => {
    render(
      <ResourceSelector kind="member" selectionMode="multiple" value={["u1"]} onChange={() => {}} source={syncSource} />,
    );
    expect(await screen.findByRole("button", { name: /remove alice nguyen/i })).toBeInTheDocument();
  });

  it("renders an avatar for the user/member kinds", async () => {
    const { container } = render(<ResourceSelector kind="user" value="" onChange={() => {}} source={syncSource} />);
    await screen.findByText("Alice Nguyen");
    expect(container.querySelector(".avatar")).toBeInTheDocument();
  });

  it("loads from an async source with the typed query", async () => {
    const load = vi.fn<(query: string, signal: AbortSignal) => Promise<ResourceOption[]>>(async () => members);
  ...
  it("serializes selected ids into a hidden field", async () => {
    const { container } = render(
      <ResourceSelector kind="member" selectionMode="multiple" name="userIds" value={["u1", "u2"]} onChange={() => {}} source={syncSource} />,
    );
