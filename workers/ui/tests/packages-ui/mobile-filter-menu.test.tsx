...
import { MobileFilterMenu } from "@id/ui";
...
const defaultGroups = [
  { key: "role", label: "Role", options: roleOptions, value: "all", onChange: defaultOnChange },
  { key: "status", label: "Status", options: statusOptions, value: "all", onChange: defaultOnChange },
];
...
describe("MobileFilterMenu", () => {
  ...
  it("shows active filter label in the trigger when one filter is set", () => {
    const groups = [
      { key: "role", label: "Role", options: roleOptions, value: "admin", onChange: vi.fn<(value: string) => void>() },
      { key: "status", label: "Status", options: statusOptions, value: "all", onChange: vi.fn<(value: string) => void>() },
    ];
  ...
  it("shows joined filter labels when two filters are active", () => {
    const groups = [
      { key: "role", label: "Role", options: roleOptions, value: "admin", onChange: vi.fn<(value: string) => void>() },
      { key: "status", label: "Status", options: statusOptions, value: "banned", onChange: vi.fn<(value: string) => void>() },
    ];
    render(<MobileFilterMenu groups={groups} />);
    const button = screen.getByRole("button", { name: "Filters" });
  ...
  it("shows X applied when three or more filters are active", () => {
    ...
    const groups = [
      { key: "a", label: "A", options: [{ value: "all", label: "All" }, { value: "x", label: "X" }], value: "x", onChange },
      { key: "b", label: "B", options: [{ value: "all", label: "All" }, { value: "y", label: "Y" }], value: "y", onChange },
      { key: "c", label: "C", options: [{ value: "all", label: "All" }, { value: "z", label: "Z" }], value: "z", onChange },
    ];
    render(<MobileFilterMenu groups={groups} />);
    const button = screen.getByRole("button", { name: "Filters" });
  ...
  it("opens a menu on click and calls onChange on menu item selection", async () => {
    ...
    const groups = [
      { key: "role", label: "Role", options: roleOptions, value: "all", onChange: roleOnChange },
      { key: "status", label: "Status", options: statusOptions, value: "all", onChange: statusOnChange },
    ];
    ...
    const adminItem = await screen.findByRole("menuitem", { name: "Role: Admin" });
  ...
  it("shows menu items for all non-all options", async () => {
    const groups = [
      { key: "role", label: "Role", options: roleOptions, value: "all", onChange: vi.fn<(value: string) => void>() },
      { key: "status", label: "Status", options: statusOptions, value: "all", onChange: vi.fn<(value: string) => void>() },
    ];
    render(<MobileFilterMenu groups={groups} />);
    const button = screen.getByRole("button", { name: "Filters" });
    ...
    expect(screen.getByRole("menuitem", { name: /Role: Admin/i })).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: /Role: User/i })).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: /Status: Active/i })).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: /Status: Banned/i })).toBeInTheDocument();
  });
  ...
  it("does not show all-option entries from filter groups in the menu", async () => {
    ...
    expect(screen.queryByRole("menuitem", { name: "Role: All Roles" })).not.toBeInTheDocument();
    expect(screen.queryByRole("menuitem", { name: "Status: All" })).not.toBeInTheDocument();
  });
  ...
  it("resets all filters when the All menu item is clicked", async () => {
    ...
    const statusOnChange = vi.fn<(value: string) => void>();
    const groups = [
      { key: "role", label: "Role", options: roleOptions, value: "admin", onChange: roleOnChange },
      { key: "status", label: "Status", options: statusOptions, value: "banned", onChange: statusOnChange },
    ];
