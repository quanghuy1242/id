...
import { ResponsiveActions } from "@id/ui";
...
function currentContainerWidth() {
  return typeof measuredContainerWidth === "function" ? measuredContainerWidth() : measuredContainerWidth;
}

function getMockBoundingClientRect(this: HTMLElement): DOMRect {
  if (this.hasAttribute?.("data-responsive-action") || this.hasAttribute?.("data-responsive-measure-action")) {
    return { x: 0, y: 0, width: 80, height: 32, top: 0, right: 80, bottom: 32, left: 0, toJSON: () => ({}) } as DOMRect;
  }
  if (this.hasAttribute?.("data-responsive-menu") || this.hasAttribute?.("data-responsive-measure-menu")) {
    return { x: 0, y: 0, width: 40, height: 32, top: 0, right: 40, bottom: 32, left: 0, toJSON: () => ({}) } as DOMRect;
  }
  if (this.querySelector?.("[data-responsive-action]")) {
    const width = currentContainerWidth();
    return { x: 0, y: 0, width, height: 32, top: 0, right: width, bottom: 32, left: 0, toJSON: () => ({}) } as DOMRect;
  }
...
function mockViewport(matches: boolean | (() => boolean)) {
  Object.defineProperty(window, "matchMedia", {
    ...
    value: vi.fn<(query: string) => MediaQueryList>().mockImplementation((query: string) => ({
      ...
      media: query,
      onchange: null,
      addListener: vi.fn<(listener: (this: MediaQueryList, ev: MediaQueryListEvent) => void) => void>(),
      removeListener: vi.fn<(listener: (this: MediaQueryList, ev: MediaQueryListEvent) => void) => void>(),
      addEventListener: vi.fn<(type: string, listener: EventListenerOrEventListenerObject | null) => void>(),
      removeEventListener: vi.fn<(type: string, listener: EventListenerOrEventListenerObject | null) => void>(),
...
describe("ResponsiveActions", () => {
  it("renders one visible action directly", () => {
    ...
    render(<ResponsiveActions actions={[{ id: "edit", label: "Edit Profile", onAction }]} />);
    expect(screen.queryByRole("button", { name: "Actions" })).toBeNull();
    expect(screen.getByRole("button", { name: "Edit Profile" }).parentElement).toHaveClass("whitespace-nowrap");
    ...
    render(
      <ResponsiveActions
        ...
        actions={[
          ...
          { id: "delete", label: "Delete User", variant: "danger", onAction: onDelete },
        ]}
      />,
    );
    ...
    render(
      <ResponsiveActions
        ...
        actions={[
          ...
          { id: "delete", label: "Delete User", variant: "danger", onAction: vi.fn<() => void>() },
        ]}
      />,
    );
