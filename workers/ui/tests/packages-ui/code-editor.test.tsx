...
import { CodeEditor } from "@id/ui";

describe("CodeEditor", () => {
  it("renders a textarea with the value", () => {
    render(<CodeEditor value='{"a":1}' onChange={() => {}} label="Metadata" />);
    expect(screen.getByRole("textbox", { name: /metadata/i })).toHaveValue('{"a":1}');
  });
  ...
  it("surfaces an error and applies the error class", () => {
    render(<CodeEditor value="{" onChange={() => {}} label="Metadata" error="Invalid JSON" />);
    expect(screen.getByRole("alert")).toHaveTextContent("Invalid JSON");
    expect(screen.getByRole("textbox", { name: /metadata/i })).toHaveClass("textarea-error");
  });
