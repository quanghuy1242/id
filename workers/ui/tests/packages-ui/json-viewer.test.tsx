...
import { JsonViewer } from "@id/ui";

describe("JsonViewer", () => {
  it("renders and highlights an object value", () => {
    const { container } = render(<JsonViewer value={{ name: "Content API", count: 3, active: true }} />);
  ...
  it("renders a label and an action slot", () => {
    render(<JsonViewer value={{ a: 1 }} label="Public JWK" action={<button>Copy</button>} />);
