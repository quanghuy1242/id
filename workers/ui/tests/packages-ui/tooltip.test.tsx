...
import { Button, Tooltip } from "@id/ui";
...
describe("Button tooltip prop", () => {
  it("exposes its tooltip on keyboard focus", async () => {
    render(<Button iconName="Trash2" ariaLabel="Delete" tooltip="Delete application" />);
    focusViaKeyboard(screen.getByRole("button", { name: /delete/i }));
    await waitFor(() => {
      expect(screen.getByRole("tooltip")).toHaveTextContent("Delete application");
    });
  });
