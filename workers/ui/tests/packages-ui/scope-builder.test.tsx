...
import { ScopeBuilder, defaultScopeValidate } from "@id/ui";
...
describe("ScopeBuilder", () => {
  it("renders selected scopes as chips", () => {
    render(<ScopeBuilder label="Scopes" value={["openid", "profile"]} onChange={() => {}} suggestions={suggestions} />);
    expect(screen.getByRole("button", { name: /remove openid/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /remove profile/i })).toBeInTheDocument();
  });

  it("removes a scope when the chip remove button is pressed", () => {
    const onChange = vi.fn<(v: string[]) => void>();
    render(<ScopeBuilder label="Scopes" value={["openid", "profile"]} onChange={onChange} suggestions={suggestions} />);
  ...
  it("excludes already-selected scopes from the suggestion list", () => {
    render(<ScopeBuilder label="Scopes" value={["openid"]} onChange={() => {}} suggestions={suggestions} />);
  ...
  it("adds a scope when a suggestion is selected", () => {
    const onChange = vi.fn<(v: string[]) => void>();
    render(<ScopeBuilder label="Scopes" value={[]} onChange={onChange} suggestions={suggestions} />);
  ...
  it("adds a valid custom scope with Enter when custom values are allowed", () => {
    const onChange = vi.fn<(v: string[]) => void>();
    render(<ScopeBuilder label="Scopes" value={[]} onChange={onChange} suggestions={suggestions} allowCustom />);
    const input = screen.getByRole("searchbox", { name: /search scopes/i });
  ...
  it("removes the last chip on Backspace from an empty input", () => {
    const onChange = vi.fn<(v: string[]) => void>();
    render(<ScopeBuilder label="Scopes" value={["openid", "profile"]} onChange={onChange} suggestions={suggestions} />);
    fireEvent.keyDown(screen.getByRole("searchbox", { name: /search scopes/i }), { key: "Backspace" });
    expect(onChange).toHaveBeenCalledWith(["openid"]);
  });

  it("serializes selected scopes into a hidden space-joined field", () => {
    const { container } = render(
      <ScopeBuilder label="Scopes" name="scope" value={["openid", "profile"]} onChange={() => {}} suggestions={suggestions} />,
    );
