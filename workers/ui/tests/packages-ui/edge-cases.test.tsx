...
import { Badge, Button, Text, Stack, Inline } from "@id/ui";
...
describe("UI component edge cases", () => {
  ...
  it("Stack renders with a single child", () => {
    const { container } = render(<Stack><span>One</span></Stack>);
  ...
  it("Button does not call onClick when disabled", () => {
    let called = false;
    const { container } = render(<Button disabled onClick={() => { called = true; }}>Disabled</Button>);
    const button = container.querySelector("button");
    button?.click();
  ...
  it("multiple inline elements align correctly", () => {
    ...
    expect(container.firstChild).toHaveClass("flex", "flex-row", "items-center", "gap-4");
