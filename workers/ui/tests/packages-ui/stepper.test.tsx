...
import { Stepper, type Step } from "@id/ui";
...
describe("Stepper", () => {
  it("renders step labels and the active step content", () => {
    render(<Stepper steps={steps} activeStep={0} onStepChange={() => {}} onComplete={() => {}} />);
  ...
  it("advances with Next", () => {
    ...
    render(<Stepper steps={steps} activeStep={0} onStepChange={onStepChange} onComplete={() => {}} />);
  ...
  it("disables Next when the active step is invalid", () => {
    const guarded: Step[] = [{ ...steps[0], isValid: false }, steps[1], steps[2]];
    render(<Stepper steps={guarded} activeStep={0} onStepChange={() => {}} onComplete={() => {}} />);
    expect(screen.getByRole("button", { name: /next/i })).toBeDisabled();
  });

  it("shows Complete on the last step and calls onComplete", () => {
    const onComplete = vi.fn<() => void>();
    render(<Stepper steps={steps} activeStep={2} onStepChange={() => {}} onComplete={onComplete} completeLabel="Create" />);
  ...
  it("jumps back when a completed step is clicked", () => {
    const onStepChange = vi.fn<(n: number) => void>();
    render(<Stepper steps={steps} activeStep={2} onStepChange={onStepChange} onComplete={() => {}} />);
    fireEvent.click(screen.getByRole("button", { name: "Type" }));
    expect(onStepChange).toHaveBeenCalledWith(0);
  });

  it("applies compact step and button sizing when requested", () => {
    const { container } = render(<Stepper steps={steps} activeStep={0} onStepChange={() => {}} onComplete={() => {}} size="sm" />);
