...
import { ResponsiveBreadcrumb } from "@id/ui";
...
describe("ResponsiveBreadcrumb", () => {
  it("renders breadcrumb items without a leading selector", () => {
    ...
    expect(screen.queryByRole("button", { name: "Show more breadcrumbs" })).toBeNull();
  });
  ...
  it("keeps the leading selector visible while collapsing earlier breadcrumb items", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Show more breadcrumbs" })).toBeInTheDocument());
    expect(screen.getByRole("button", { name: "Acme Publishing" })).toBeInTheDocument();
    expect(screen.getByText("Members")).toBeInTheDocument();
    expect(screen.getByText("Identity").closest("li")).toHaveStyle({ display: "none" });
  });
});
