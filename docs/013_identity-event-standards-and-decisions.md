...
| A4 ✓ | Migrate `content-api` user/team/admin checks to SCIM and service-account attach flows to doc 018's OAuth-client contract | **Implemented 2026-05-27** — `ScimContentPrincipalDirectory` replaces `IdContentPrincipalDirectory`; content-api calls SCIM GET + OAuth picker GET; SCIM audience aligned to `/system` for unified M2M token; 128 tests pass | docs 017 + 018 |
