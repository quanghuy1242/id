...
export function FileDropzone({ label, accept, onFiles, multiple, maxSizeBytes, hint }: FileDropzoneProps) {
  const [rejected, setRejected] = useState<string | undefined>(undefined);

  function acceptFiles(files: File[]) {
    const tooBig = maxSizeBytes !== undefined && files.some((f) => f.size > maxSizeBytes);
  ...
  async function handleDrop(items: { kind: string; getFile: () => Promise<File> }[]) {
    const files = await Promise.all(items.filter((i) => i.kind === "file").map((i) => i.getFile()));
