...
import { useRadioGroupState, type RadioGroupState, useToggleState } from "react-stately";
...
export function RadioGroup({
  ...
  return (
    <div {...radioGroupProps} className="fieldset">
      <span {...labelProps} className="fieldset-legend text-base font-medium text-base-content">
        {title}
      </span>
      {options.map((option) => (
        <RadioItem
...
function RadioItem({
  ...
  const { inputProps, labelProps } = useRadio({ value, "aria-label": label }, state, ref);
...
export function Checkbox({
  ...
  return (
    <div className="form-control w-full">
      <label {...labelProps} className="label cursor-pointer justify-start gap-3">
        <input
          {...inputProps}
          ref={ref}
