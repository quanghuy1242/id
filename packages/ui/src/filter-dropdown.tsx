...
export function FilterDropdown({ label, options, value, onChange, size = "md", className, showLabel }: FilterDropdownProps) {
  return (
    <div className={showLabel ? "form-control w-full" : undefined}>
      ...
      >
        <SelectTrigger className={`${sizeClass[size]} ${showLabel ? "w-full" : "w-auto"} bg-none flex items-center gap-1`}>
          {!showLabel ? <span className="text-base-content/50 mr-0.5">{label}:</span> : null}
          <SelectValue />
