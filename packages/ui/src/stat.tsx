...
export function StatGroup({ children, columns = "auto", layout = "grid", density = "comfortable", frame = "standalone" }: StatGroupProps) {
  if (layout === "inline") {
    return (
  ...
  const densityClass = density === "compact" ? "[&_.stat]:px-4 [&_.stat]:py-3 [&_.stat-value]:text-xl" : "";
  const frameClass = frame === "seamless" ? "" : "overflow-hidden rounded-box border border-base-300";
  return (
    <div
