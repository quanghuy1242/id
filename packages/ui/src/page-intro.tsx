 ...
 */
export function PageIntro({ title, description, info, infoTitle, actions }: PageIntroProps) {
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex min-w-0 flex-col gap-1">
        <div className="flex items-center gap-1.5">
          ...
          {info ? (
            <InfoPopover title={infoTitle ?? title} label={`About ${title}`} placement="bottom" size="sm">
              {info}
            </InfoPopover>
          ) : null}
        </div>
