...
export function Stepper({ steps, activeStep, onStepChange, onComplete, completeLabel = "Complete", size = "md" }: StepperProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {isLast ? (
          <Button variant="primary" size={size} disabled={!canAdvance || submitting} onClick={() => void handleComplete()}>
            {completeLabel}
          </Button>
        ) : (
          <Button variant="primary" size={size} disabled={!canAdvance} onClick={() => onStepChange(activeStep + 1)}>
            Next
          </Button>
        )}
      </div>
