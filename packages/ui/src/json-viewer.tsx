...
const maxHeightClass: Record<NonNullable<JsonViewerProps["maxHeight"]>, string> = {
  sm: "max-h-40",
  md: "max-h-72",
  lg: "max-h-96",
};
...
export function highlightJson(json: string): ReactNode[] {
  ...
  for (const match of json.matchAll(tokenPattern)) {
    ...
    if (start > lastIndex) out.push(<Fragment key={key++}>{json.slice(lastIndex, start)}</Fragment>);
  ...
  if (lastIndex < json.length) out.push(<Fragment key={key++}>{json.slice(lastIndex)}</Fragment>);
  return out;
}

export function JsonViewer({ value, label, maxHeight = "md", action }: JsonViewerProps) {
  let json: string;
