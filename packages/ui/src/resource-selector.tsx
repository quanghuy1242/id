...
export type ResourceSource =
  | { readonly mode: "async"; readonly load: (query: string, signal: AbortSignal) => Promise<ResourceOption[]> }
...
function defaultRenderOption(kind: ResourceKind, option: ResourceOption): ReactNode {
  ...
  return (
    <div className="flex items-center gap-3">
      {withAvatar ? <Avatar size="sm" image={option.image ?? undefined} initials={option.label} /> : null}
      <div className="flex min-w-0 flex-col">
        <span className="truncate text-base-content">{option.label}</span>
        {option.sublabel ? <span className="truncate text-xs text-base-content/50">{option.sublabel}</span> : null}
      </div>
      {option.badge ? <span className="badge badge-outline badge-sm ml-auto">{option.badge}</span> : null}
    </div>
  );
}
...
export function ResourceSelector({
  ...
  const hidden = new Set([...excludeIds, ...(selectionMode === "multiple" ? selectedIds : [])]);
  ...
  const filteredItems =
    source.mode === "sync" && query !== ""
      ? sourceItems.filter((o) =>
          [o.label, o.sublabel, o.badge].some((text) => text ? contains(text, query) : false),
        )
      : sourceItems;
