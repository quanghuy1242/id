...
function elementWidth(element: HTMLElement) {
  const measuredWidth = element.offsetWidth || element.getBoundingClientRect().width;
...
function nextCollapseCount(
  ...
  const allActionsWidth = actionWidths.reduce((total, width) => total + width, 0) + Math.max(0, actionWidths.length - 1) * gap;
  if (allActionsWidth <= availableWidth) return 0;

  for (let collapseCount = 1; collapseCount <= actionWidths.length; collapseCount++) {
    const directCount = actionWidths.length - collapseCount;
    const directWidth = actionWidths.slice(0, directCount).reduce((total, width) => total + width, 0);
    const visibleCount = directCount + 1;
    const totalWidth = directWidth + menuWidth + Math.max(0, visibleCount - 1) * gap;
...
export function ResponsiveActions({
  ...
  const visibleActions = useMemo(() => actions.filter((action) => !action.isHidden), [actions]);
  const actionSignature = visibleActions.map((action) => `${action.id}:${action.label}:${action.variant ?? ""}`).join("|");
  ...
  const measure = useCallback(() => {
    ...
    const actionItems = measureList.querySelectorAll<HTMLElement>("[data-responsive-measure-action]");
    const menuItem = measureList.querySelector<HTMLElement>("[data-responsive-measure-menu]");
    const gap = Number.parseFloat(window.getComputedStyle(measureList).columnGap || window.getComputedStyle(measureList).gap || "0") || 0;
    const actionWidths = Array.from(actionItems).map((item) => elementWidth(item));
  ...
  useSafeLayoutEffect(() => {
    if (visibleActions.length <= 1 || collapseCount >= visibleActions.length) return;
    ...
    if (list.scrollWidth > available + 1) {
      setCollapseCount((current) => Math.min(visibleActions.length, current + 1));
    }
  }, [collapseCount, visibleActions.length]);

  const directCount = visibleActions.length <= 1 ? visibleActions.length : Math.max(0, visibleActions.length - collapseCount);
  const collapsedActions = visibleActions.slice(directCount);
  const actionById = new Map(collapsedActions.map((action) => [action.id, action]));
    ...
    >
      ...
      >
        {visibleActions.map((action) => (
          <span key={action.id} data-responsive-measure-action="" className="shrink-0 whitespace-nowrap">
            <Button
        ...
        <span data-responsive-measure-menu="" className="shrink-0 whitespace-nowrap">
          <Button variant="ghost" size={size} iconName="Ellipsis" ariaLabel={ariaLabel} disabled />
        </span>
      </div>
      <div ref={listRef} className="flex min-w-0 items-center justify-end gap-2">
        ...
        >
          <MenuTrigger>
            <Button variant="ghost" size={size} iconName="Ellipsis" ariaLabel={ariaLabel} tooltip="More actions" />
            <Menu
