...
const maxHeightClass: Record<NonNullable<CodeBlockProps["maxHeight"]>, string> = {
  sm: "max-h-40",
  md: "max-h-72",
  lg: "max-h-96",
};

export function CodeBlock({ label, value, action, maxHeight = "md" }: CodeBlockProps) {
  return (
    <div className="overflow-hidden rounded-box border border-base-300 bg-base-200">
      ...
      <pre className={`m-0 overflow-auto p-3 font-mono text-sm leading-relaxed text-base-content whitespace-pre ${maxHeightClass[maxHeight]}`}>
        <code>{value}</code>
      </pre>
    </div>
  );
