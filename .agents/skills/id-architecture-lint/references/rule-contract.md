...
- `architecture/no-magic-numbers`: numeric literals in non-infrastructure core source layers (`application`, `domain`, `http`, `shared`, `auth`, `composition`, and `config`) and `packages/lib/src` must be extracted to named constants. 0 and 1 are exempt as universal base values. SCREAMING_SNAKE_CASE `const` declarations, property keys, enum members, and type annotations are exempt as definition sites.
- `architecture/constants-placement`: SCREAMING_SNAKE_CASE const declarations must live in `src/shared/`, `src/domain/`, `src/infrastructure/`, `workers/core/src/auth/config.ts`, or `packages/lib/src/`. They are forbidden in ordinary core `application`, `http`, `auth` adapter/plugin, `composition`, and `config` files.
...
- `architecture/no-direct-db-access`: raw D1 `.prepare()`, `.batch()`, `.exec()` is forbidden outside `infrastructure/`, `auth/cli.ts`, and the exact resource-server audience companion at `auth/plugins/resource-server/audiences.ts`. Use Better Auth adapter APIs for plugin CRUD.
- `architecture/plugin-owned-table-boundary`: plugin-owned table model constants must not be imported into generic `infrastructure/persistence/**`; runtime exceptions for plugin tables stay inside the owning plugin directory.
