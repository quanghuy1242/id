...
| `NumberInput` | `label`, `name?`, `value?: number\|null`, `defaultValue?: number\|null`, `onChange?: (v: number\|null) => void`, `minValue?`, `maxValue?`, `step?`, `required?`, `size?`, `description?`, `placeholder?`, `formatOptions?` | React Aria `NumberField` + DaisyUI `input` with `−`/`+` steppers. Use instead of a free-text box for any numeric field (quota, limits, counts). Empty value is `null`; hidden field is the number string or `""`. `(Optional)` hint shows when not `required`. |
| `DateTimeInput` | `label`, `name?`, `value?: number\|null` (epoch ms), `defaultValue?: number\|null`, `onChange?: (v: number\|null) => void`, `mode?: "datetime"\|"date"`, `required?`, `size?`, `description?` | React Aria `DatePicker` (segmented `DateInput` + DaisyUI-styled `Calendar` popover). Pick dates/times — never ask the operator to type ISO strings. Value is epoch ms in local time; hidden field is the ms string or `""`. |
| `TagInput` | `label`, `value: string[]`, `onChange`, `name?`, `placeholder?`, `validate?`, `normalize?`, `separator?` (default `" "`), `size?`, `description?` | Free-text chips on React Aria `TagGroup` + `TextField`; Enter or comma commits, Backspace on empty removes the last. For free-form value lists (email domains, tags). Exports `defaultDomainValidate`. Hidden field joins with `separator`. |
