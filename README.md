...
- [docs/034_email-templating.md](docs/034_email-templating.md) — DB-backed email templating proposal: the single render seam, an `idEmailTemplates` BA plugin/table with KV+D1 resolution and hardcoded fallback, a zero-dependency autoescaped `{{var}}` interpolator over a per-kind allowlist (with the Liquid/Handlebars SSTI evidence behind the choice), a developer-owned email-safe base layout, and the admin editor with preview/test-send
