...
var UI_ALLOWED_APP_ROUTE_PREFIXES = ["admin/", "login/", "consent/", "select-authorization-context/", "ui-health/"];
  ...
  create: function (context) {
    ...
    return {
      Program: function (node) {
        ...
        context.report({
          node: node,
          message: "ui-id may only define public App Router routes under workers/ui/src/app/{admin,login,consent,select-authorization-context,ui-health}/**. Core owns /api/* and root health/auth routes; keep UI-owned routes under /login, /consent, /select-authorization-context, /ui-health, or /admin/*.",
        });
      },
      CallExpression: function (node) {
        var info = extractRoutePathFromAppCall(node);
