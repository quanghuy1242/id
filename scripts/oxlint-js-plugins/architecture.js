...
var UI_ALLOWED_APP_ROUTE_PREFIXES = ["admin/", "account/", "login/", "register/", "consent/", "select-authorization-context/", "forgot-password/", "reset-password/", "verify-email/", "ui-health/"];
  ...
  create: function (context) {
    ...
    return {
      Program: function (node) {
        ...
        context.report({
          node: node,
          message: "ui-id may only define public App Router routes under workers/ui/src/app/{admin,account,login,register,consent,select-authorization-context,forgot-password,reset-password,verify-email,ui-health}/**. Core owns /api/* and root health/auth routes; keep UI-owned routes under /account, /login, /register, /consent, /select-authorization-context, /forgot-password, /reset-password, /verify-email, /ui-health, or /admin/*.",
        });
      },
      CallExpression: function (node) {
        var info = extractRoutePathFromAppCall(node);
